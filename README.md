...
- The Mobile MCP bridge requires `MOBILE_MCP_BRIDGE_TOKEN` for protected endpoints. `npm run runtime:mobile-mcp` now fails closed when the token is missing. For isolated local development only, set `MOBILE_MCP_ALLOW_INSECURE_DEV=true` explicitly to run without a token.
...
- `npm run setup:mobile-mcp:local` installs Mobile MCP bridge dependencies and stores required local secret/config values in Windows User env through hidden prompts, including a generated `MOBILE_MCP_BRIDGE_TOKEN` when one is not already configured; pass `-- -EnsureOperatorAfter` to create/fix the UI smoke operator, `-- -SyncDevicesAfter` to sync DB devices in the same setup process, `-- -QuickVerifyAfter` for non-browser verification, or `-- -VerifyAfter` for full browser verification when runtime services are already healthy
